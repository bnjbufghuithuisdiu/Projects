...
extern int zhencount;
extern u16 display[76800];
DCMI_InitTypeDef DCMI_InitStructure;

//DCMI中断服务函数
void DMA2_Stream1_IRQHandler(void)//DMA2_Stream1_IRQHandler DCMI_IRQHandler
{
	long i=0;
 if(DMA_GetITStatus(DMA2_Stream1,DMA_IT_TCIF1) != RESET)
	{
	  ...
	  zhencount++; //图像分段标志，共计2段
		if(zhencount==1)
		{
			...
			while(i< 38400)
			{
				if(Parallel_Data_Buffer1[i]>0x7fff)
				{
					display[i]=	0xffff;
				}
				else
		...
		if(zhencount==2)
		{
			...
			while(i< 38400)
			{
				if(Parallel_Data_Buffer2[i]>0x7fff) //0x7fff is the threshold value,you can change it.

				{
					display[38400+i]=	0xffff;
				}
				else
				{
					display[38400+i]=	0x0000;
				}
				i++;				
			}
		}
...
{ 
  ...
  DMA_Init(DMA2_Stream1, &DMA_InitStructure);//初始化DMA Stream
	...
	DMA_DoubleBufferModeConfig(DMA2_Stream1,(u32)Parallel_Data_Buffer1,DMA_Memory_0);
...
void My_DCMI_Init(void)
{
  ...
  NVIC_InitStructure.NVIC_IRQChannel = DMA2_Stream1_IRQn;//DCMI_IRQn;
	NVIC_InitStructure.NVIC_IRQChannelPreemptionPriority=1;//抢占优先级1
